    ...
    $.xmpp ={
        ...
        */
        ...
        loginToken: function(options){
            ...
            var domain = split[1];
            var xmpp = this;
            var text = "<body rid='"+this.rid+"' xmlns='http://jabber.org/protocol/httpbind' sid='"+this.sid+"'><auth xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='BBSAUTH'>"+Base64.encode(user+"\u0000"+user+"\u0000"+options.token)+"</auth></body>";
            var url = this.url;
            $.post(this.url,text,function(data){
                var response = $(xmpp.fixBody(data));
                if(response.find("success").length){
